"""
Search for exact match to paper results.
Key insight: threshold slightly above 0.160 gives insignificant high-debt effect.
"""
...
def main():
    ...
    print("\n" + "="*90)
    print("SEARCH: Fine-grained around threshold 0.160-0.162")
    print("="*90)
    
    results = []
    
    # Try different sample periods
    starts = ['2008-10', '2008-11', '2008-12', '2009-01', '2009-02', '2009-03']
    ends = ['2014-06', '2014-07', '2014-08', '2014-09', '2014-10', '2014-11', '2014-12', '2015-01', '2015-02']
    ...
    for start_str in starts:
        ...
        for end_str in ends:
            ...
            for thresh in np.arange(0.158, 0.165, 0.0005):
                for scale in np.arange(2, 8, 0.1):
    ...
    if results:
        ...
        print(f"\nTop 40 (sorted by coefficient match to -9.4/-3.5):")
        print(f"{'Start':<10} {'End':<10} {'Thresh':>7} {'Scale':>6} {'Low β':>7} {'p':>6} {'High β':>7} {'p':>6} {'N':>8} {'Err':>5}")
        print("="*95)
        
        for r in results[:40]:
            print(f"{r['start']:<10} {r['end']:<10} {r['thresh']:>7.4f} {r['scale']:>6.2f} {r['low_coef']:>6.1f}* {r['low_pval']:>5.3f} {r['high_coef']:>6.1f}  {r['high_pval']:>5.3f} {r['low_n']:>3}/{r['high_n']:<3} {r['coef_err']:>5.1f}")
        ...
        print(f"  Low-debt: {best['low_coef']:.1f} bps (p={best['low_pval']:.3f})")
        print(f"  High-debt: {best['high_coef']:.1f} bps (p={best['high_pval']:.3f})")
        print(f"  N: {best['low_n']}/{best['high_n']}")
        
        # Check if any match N=40/23
        n_match = [r for r in results if r['low_n'] == 40 and r['high_n'] == 23]
        if n_match:
            print(f"\n\nWith exact N=40/23 ({len(n_match)} found):")
            n_match.sort(key=lambda x: x['coef_err'])
            for r in n_match[:10]:
                print(f"{r['start']:<10} {r['end']:<10} {r['thresh']:>7.4f} {r['scale']:>6.2f} {r['low_coef']:>6.1f}* {r['low_pval']:>5.3f} {r['high_coef']:>6.1f}  {r['high_pval']:>5.3f}")
