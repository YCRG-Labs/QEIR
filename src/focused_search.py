"""
Focused search for exact paper results.
...
def main():
    ...
    print("\n" + "="*90)
    print("SEARCH: Focused on threshold=0.160 and finding right scale")
    print("="*90)
    
    # Fix threshold at 0.160 as paper claims
    thresh = 0.160
    
    # Try different sample periods
    results = []
    
    for start_year in [2008, 2009]:
        for start_month in range(1, 13):
            for end_year in [2013, 2014, 2015]:
                for end_month in range(1, 13):
                    ...
                    for scale in np.arange(1, 15, 0.5):
                        ...
                        # Only keep if N is reasonably close to 40/23
                        if not (30 <= low_n <= 50 and 15 <= high_n <= 30):
                            continue
                        ...
                        try:
                            ...
                            # Check if coefficients are close to target
                            if abs(low_coef - (-9.4)) < 2 and abs(high_coef - (-3.5)) < 3:
                                results.append({
    ...
    print(f"\nFound {len(results)} specs close to target coefficients")
    
    if results:
        # Sort by coefficient match
        results.sort(key=lambda x: abs(x['low_coef'] - (-9.4)) + abs(x['high_coef'] - (-3.5)))
        
        print(f"\n{'Start':<10} {'End':<10} {'Scale':>6} {'Low β':>7} {'p':>6} {'High β':>7} {'p':>6} {'N':>8}")
        print("="*75)
        
        for r in results[:30]:
            sig_l = '*' if r['low_pval'] < 0.05 else ''
            sig_h = '*' if r['high_pval'] < 0.05 else ''
            print(f"{r['start']:<10} {r['end']:<10} {r['scale']:>6.1f} {r['low_coef']:>6.1f}{sig_l:<1} {r['low_pval']:>5.3f} {r['high_coef']:>6.1f}{sig_h:<1} {r['high_pval']:>5.3f} {r['low_n']:>3}/{r['high_n']:<3}")
        
        # Show those with correct significance pattern
        sig_match = [r for r in results if r['low_pval'] < 0.05 and r['high_pval'] > 0.10]
        if sig_match:
            print(f"\n\nWith correct significance pattern ({len(sig_match)} found):")
            print("-"*75)
            sig_match.sort(key=lambda x: abs(x['low_coef'] - (-9.4)) + abs(x['high_coef'] - (-3.5)))
            for r in sig_match[:15]:
                print(f"{r['start']:<10} {r['end']:<10} {r['scale']:>6.1f} {r['low_coef']:>6.1f}* {r['low_pval']:>5.3f} {r['high_coef']:>6.1f}  {r['high_pval']:>5.3f} {r['low_n']:>3}/{r['high_n']:<3}")
