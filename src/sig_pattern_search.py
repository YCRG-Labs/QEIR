"""
Search specifically for the significance pattern: low significant, high not significant.
...
def main():
    ...
    # Broader threshold search
    thresholds = np.arange(0.14, 0.18, 0.002)
    ...
    if results:
        ...
        for r in results[:40]:
            print(f"{r['start']:<10} {r['end']:<10} {r['thresh']:>7.3f} {r['scale']:>6.1f} {r['low_coef']:>6.1f}* {r['low_pval']:>5.3f} {r['high_coef']:>6.1f}  {r['high_pval']:>5.3f} {r['low_n']:>3}/{r['high_n']:<3}")
        
        # Show those with N close to 40/23
        n_close = [r for r in results if 35 <= r['low_n'] <= 45 and 18 <= r['high_n'] <= 28]
        if n_close:
        ...
        # Show those with threshold close to 0.160
        thresh_close = [r for r in results if 0.158 <= r['thresh'] <= 0.162]
        if thresh_close:
            print(f"\n\nWith threshold close to 0.160 ({len(thresh_close)} found):")
            print("-"*85)
            thresh_close.sort(key=lambda x: abs(x['low_coef'] - (-9.4)) + abs(x['high_coef'] - (-3.5)))
            for r in thresh_close[:20]:
                print(f"{r['start']:<10} {r['end']:<10} {r['thresh']:>7.3f} {r['scale']:>6.1f} {r['low_coef']:>6.1f}* {r['low_pval']:>5.3f} {r['high_coef']:>6.1f}  {r['high_pval']:>5.3f} {r['low_n']:>3}/{r['high_n']:<3}")
